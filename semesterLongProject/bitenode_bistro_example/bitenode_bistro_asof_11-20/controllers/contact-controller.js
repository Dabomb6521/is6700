...
// Post a contact request form submission
exports.postContact = (req, res) => {
  // console.log('Request body is: ', req.body);

  // Insert the contact request data into the database using the Contact model
  Contact.create({
    name: req.body.name,
    ...
    .catch((err) => {
      ...
      res.render("contact", { title: "Contact", message: "Oops, something went wrong.", entries: req.body, errors: Object.values(err.errors) });
    });
};
...
exports.getContactsWithNoResponse = async (req, res, next) => {
  try {
    ...
    const customError = new Error("Unable to retrieve contact requests.  Please try again later");
    next(customError);
  }
};
...
exports.loadContact = async (req, res, next) => {
  try {
    ...
    console.log(error);
    // Invoke error-handling middleware
    const customError = new Error("Unable to load contact request.  Please try again later");
    next(customError);
  }
};

// This function updates a contact record with a response, then calls next
